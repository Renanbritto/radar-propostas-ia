...
async def test_rag_semantic_search_accuracy():
    results = vector_store.search("ferrovias gasodutos e industria naval", top_k=2)
    assert len(results) > 0
    top_chunk, score = results[0]
    assert "ferrovia" in top_chunk.text.lower() or "naval" in top_chunk.text.lower()
    assert top_chunk.candidate_id == "cand_pres_1"

@pytest.mark.asyncio
async def test_rag_answer_with_citations():
    request = ChatRequest(query="Quais as propostas para o SUS e saude publica?")
