...
import { Users, FileText, CheckCircle, ArrowRight, Bot, GitCompare } from "lucide-react";
    ...
    <div className="py-8 space-y-8">
      ...
      <div>
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-500/10 border border-sky-500/20 text-sky-400 text-xs font-semibold mb-3">
          ...
          <span>Diretório de Candidaturas</span>
        </div>
        <h1 className="text-3xl font-extrabold text-white tracking-tight">
          Planos de Governo Registrados no TSE
        </h1>
        <p className="text-xs sm:text-sm text-slate-300 mt-1 max-w-2xl">
          Consulte o perfil programático de cada candidato, a distribuição de propostas por eixo temático e os documentos oficiais indexados no sistema.
        </p>
      </div>

      {/* Candidate Cards Detailed Grid */}
      <div className="space-y-8">
        {candidates.map((cand) => (
          <div
            key={cand.id}
            className="liquid-glass-card rounded-3xl p-6 sm:p-8 border-l-4"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-1 space-y-4">
                <div className="flex items-center gap-4">
                  <div
                    className="w-16 h-16 rounded-2xl flex items-center justify-center font-extrabold text-2xl text-white shadow-lg"
                ...
                <div className="pt-2 flex flex-wrap gap-3">
                  <Link
                    ...
                    className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold text-xs transition-colors"
                  >
                  ...
                  <Link
                    href={`/chat`}
                    className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-white/[0.05] hover:bg-white/[0.1] text-slate-200 border border-white/[0.08] text-xs font-semibold transition-colors"
                  >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {cand.key_highlights.map((highlight, idx) => (
                      <div key={idx} className="p-3 rounded-xl bg-black/30 border border-white/[0.04] flex items-start gap-2.5">
                    ...
                    {topics.map((t) => {
                      ...
                      return (
                        <div key={t.id} className="p-2.5 rounded-xl bg-white/[0.02] border border-white/[0.04]">
                          <span className="text-[11px] text-slate-400 block truncate">{t.name}</span>
