...
import { DollarSign, ShieldAlert, AlertTriangle, TrendingUp, Building, ArrowUpRight, CheckCircle2 } from "lucide-react";
    ...
    <div className="py-8 space-y-8">
      ...
      <div>
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold mb-3">
          ...
          <span>InvestigaVoto • Auditoria de Finanças Eleitorais</span>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="liquid-glass-card rounded-2xl p-5">
              <span className="text-xs text-slate-400 font-medium">Total de Recursos Declarados</span>
            ...
            <div className="liquid-glass-card rounded-2xl p-5">
              <span className="text-xs text-slate-400 font-medium">Total de Despesas Contratadas</span>
              <p className="text-xl sm:text-2xl font-bold text-sky-400 font-mono mt-1">
                R$ {(data.total_campaign_expenses / 1e6).toFixed(1)}M
              </p>
              <span className="text-[10px] text-slate-400">Execução orçamentária</span>
            </div>
            <div className="liquid-glass-card rounded-2xl p-5">
              <span className="text-xs text-slate-400 font-medium">Alertas de Auditoria</span>
              <p className="text-xl sm:text-2xl font-bold text-amber-400 font-mono mt-1">
                {data.total_anomalies_flagged}
              </p>
              <span className="text-[10px] text-amber-300">Detecção forense</span>
            </div>
            <div className="liquid-glass-card rounded-2xl p-5">
              <span className="text-xs text-slate-400 font-medium">Índice de Transparência</span>
          ...
          {selectedCand && (
            <div className="space-y-6">
              ...
              <div className="liquid-glass-card rounded-2xl p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-sm text-white flex items-center gap-2">
                    ...
                    Maiores Fornecedores & Prestadores de Serviços
                  </h3>
                  <span className="text-xs text-slate-400 font-mono">
                    {selectedCand.top_suppliers.length} contratados mapeados
                  </span>
                </div>
                  ...
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="border-b border-white/[0.06] text-slate-400 font-mono">
                        ...
                        <th className="pb-2">% do Orçamento</th>
              ...
              {selectedCand.anomalies.length > 0 && (
                <div className="liquid-glass-card rounded-2xl p-6 border-l-4 border-amber-500 space-y-4">
                  <div className="flex items-center gap-2">
                    <AlertTriangle className="w-5 h-5 text-amber-400" />
                    <h3 className="font-bold text-sm text-white">Alertas de Auditoria Forense</h3>
                  </div>
                  ...
                  <div className="space-y-3">
                    {selectedCand.anomalies.map((anom) => (
                      <div key={anom.id} className="p-4 rounded-xl bg-black/40 border border-white/[0.04] space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-xs text-amber-300">{anom.anomaly_type}</span>
                        ...
                        <p className="text-[11px] text-slate-400 pt-1 border-t border-white/[0.04]">
                          <strong>Recomendação de Auditoria:</strong> {anom.audit_recommendation}
                        </p>
                      </div>
                    ))}
                  </div>
